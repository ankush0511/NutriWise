...
# import json
# from ingredent_agent import text_extractor,risk_alternate,risk_scoring
# from text_extraction import configure_gemini, extract_text_from_image
# uploaded_file = st.file_uploader("Upload an image", type=["png", "jpg", "jpeg"])
# configure_gemini()
# if uploaded_file is not None:
#     st.image(uploaded_file, caption="Uploaded Image.", use_container_width=True)
#     i_to_text=extract_text_from_image(uploaded_file,"extract all the text from the image")
#     resp=text_extractor.run(f"the user input is:{i_to_text}").content
#     st.write_stream(f"Extracted Ingredients: {resp}")

#     resp=risk_scoring.run(resp).content

#     risk_scoring_data  = re.findall(r"```json\s*({.*?})", resp, re.DOTALL)
#     # st.write(risk_scoring_data)
#     risk_scoring_data=json.loads(risk_scoring_data[0])
#     if risk_scoring_data:
#         st.header("⚠️ Risk Scoring")
#         allergens = risk_scoring_data.get("allergens_found", [])
#         score = risk_scoring_data.get("risk_score", None)
#         explanation = risk_scoring_data.get("explanation", "")

#         # Badge-style display
#         st.markdown("**Allergens Found:** " + ", ".join([f"`{a}`" for a in allergens]))
#         if score is not None:
#             color = "red" if score >= 0.8 else "orange" if score >= 0.5 else "green"
#             st.markdown(f"**Risk Score:** <span style='color:{color}; font-weight:bold'>{score}</span>", unsafe_allow_html=True)
#         st.write_stream(f"**Explanation:** {explanation}")


#     # st.write("Risk Scoring:", resp.content)
...
#         for alt in resp.get("alternative_suggestions", []):
#             with st.container():
#                 st.subheader(f"✅ {alt['product_name']}")
#                 st.write_stream(f"**Reason:** {alt['reason']}")
                
#                 allergen_profile = alt.get('allergen_profile', {})
#                 if isinstance(allergen_profile, dict):
#                     profile_text = ", ".join([f"{k}: {v}" for k, v in allergen_profile.items()])
#                     st.write_stream(f"**Allergen Profile:** {profile_text}")
#                 else:
#                     st.write_stream(f"**Allergen Profile:** {allergen_profile}")
#                 st.markdown("---")
